...
The exporter correlates scoped variables named `inputValue` by scope key. A service task input mapping can therefore be used to provide task-local worker input variables, and the cached variable is dropped after a successful job completion, on job deletion, or when the corresponding scope is completed or terminated.
